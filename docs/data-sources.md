...
- A dependency, locale, search query, cursor, plan digest, or source definition change creates a new request key.
...
For a Dify-like node settings panel, create one stable resolver per mounted host boundary and keep the node value controlled by the workflow editor. Replace the resolver when tenant context changes. Never use a module-global cache for organization-scoped options.
