...
export async function runCli(arguments_: string[]): Promise<number> {
  ...
  if (command === 'validate' || command === 'compile' || command === 'digest') {
    ...
    if (!result.ok || !result.document || !result.plan) {
      await emit({ ok: false, diagnostics: result.diagnostics }, options);
      return 1;
    }
    if (command === 'validate') {
      await emit(
      ...
      await emit(
        { ok: true, revision: result.document.revision, digest: result.document.digest },
        options,
      );
    return 0;
  }
  ...
  if (command === 'patch') {
    ...
    if (!documentResult.ok || !documentResult.document) {
      await emit({ ok: false, diagnostics: documentResult.diagnostics }, options);
      return 1;
    }
