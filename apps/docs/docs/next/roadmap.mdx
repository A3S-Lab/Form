...
The current `next` baseline has completed the host-neutral embedding contract, CSS isolation gates, adapter parity, Schema Profile 1, deterministic computed-rule chains, cancellable host-owned async validation, and host-owned data-source orchestration. Subscriptions, localization, and performance gates remain in progress.
