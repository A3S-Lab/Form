...
The current `next` baseline has completed the host-neutral embedding contract, CSS isolation gates, adapter parity, Schema Profile 1, and deterministic computed-rule chains with failure tracing. Async validation, data-source orchestration, subscriptions, localization, and performance gates remain in progress.
