...
- Stabilize deterministic `computed` rule evaluation and rule tracing for release.
- Stabilize cancellable async validation and stable server-error mapping for release.
...
The current `next` baseline has completed the host-neutral embedding contract, CSS isolation gates, adapter parity, Schema Profile 1, deterministic computed-rule chains, and cancellable host-owned async validation. Data-source orchestration, subscriptions, localization, and performance gates remain in progress.
