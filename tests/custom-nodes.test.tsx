...
import { workflowFormSeeds } from '../apps/playground/src/workflow-samples';
...
function WorkflowJsonHarness() {
  const template = workflowFormSeeds.find((seed) => seed.id === 'workflow-template-config');
  if (!template) throw new Error('Missing template node configuration example.');
  const [document, setDocument] = useState<FormDocument>(() => structuredClone(template.document));
  const [value, setValue] = useState<JsonObject>({});
  return (
    <>
      <FormDesigner
        ...
        nodeRegistry={playgroundNodeRegistry}
      />
      <output data-testid="workflow-json-value">{JSON.stringify(value)}</output>
    </>
  );
}
...
describe('custom form nodes', () => {
  ...
  it('edits structured workflow configuration with inline JSON validation', () => {
    render(<WorkflowJsonHarness />);

    fireEvent.click(screen.getByRole('button', { name: '预览' }));
    const editor = screen.getByLabelText('输出值（value）');
    fireEvent.change(editor, { target: { value: '{invalid' } });
    expect(screen.getByText('JSON 格式无效，请检查括号、引号和逗号。')).toBeTruthy();
