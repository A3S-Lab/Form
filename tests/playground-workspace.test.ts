...
describe('Playground workspace storage', () => {
  ...
  it('adds newly bundled examples without replacing existing local forms', () => {
    ...
    const enriched = loadPlaygroundWorkspace(
      storage,
      [{ id: 'employee-onboarding', document: sampleForm }, ...workflowFormSeeds],
      now,
    );

    expect(enriched.activeFormId).toBe('employee-onboarding');
    expect(enriched.forms).toHaveLength(11);
    expect(enriched.forms.filter((record) => record.id === 'employee-onboarding')).toHaveLength(1);
    expect(enriched.forms.map((record) => record.id)).toContain('workflow-agent-config');
    expect(enriched.forms.map((record) => record.id)).toContain('workflow-output-config');
  });

  it('upgrades an older bundled example once while preserving its original creation time', () => {
    const storage = new MemoryStorage();
    const templateSeed = workflowFormSeeds.find((seed) => seed.id === 'workflow-template-config');
    if (!templateSeed) throw new Error('Missing workflow template seed.');
    const outdatedDocument = structuredClone(templateSeed.document);
    ...
    const outdated = loadPlaygroundWorkspace(
      storage,
      [{ id: templateSeed.id, document: outdatedDocument }],
      now,
    );
